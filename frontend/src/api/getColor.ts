/**
 * Pre-calculated Tailwind-style color palettes.
 * Each color has shades 50-900.
 */
...
export default function getColor(color: string, shade: number): string {
  const shades = palette[color];
  if (!shades) return palette.slate[shade] || "#666";
  return shades[shade] || "#666";
}
