...
- **Scan scope:** Gmail/IMAP cover all mail (Gmail `before:` query with no label filter; IMAP `[Gmail]/All Mail`). Apple Mail's MCP `search_messages` proved unreliable (returns empty / non-JSON), so the runner LISTS each configured mailbox (`mail_list_messages`) and filters by sender + date in Python — default scope is INBOX, extendable per-workflow via config `mailboxes: [..]`. Listing per mailbox also yields each match's real mailbox, which the trash step needs. Coverage per run is bounded by `reaper_fetch_limit_per_sender` messages *listed* per mailbox. Sender matching is substring/contains (so `@e1.theathletic.com` matches every sub-brand from that domain).
